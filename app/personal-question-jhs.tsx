...
const PQ = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // Sort prediction scores and take only top 5
        const sortedScores = [...(data?.prediction_scores || [])].sort((a, b) => b.score - a.score).slice(0, 5);
  ...
  // Get top 5 strands for display in the chart
        ...
        <View style={{ padding: 15, backgroundColor: "#fff", borderRadius: 10, elevation: 5 }}>
          ...
          {topFiveStrands.length > 0 && (
            <View style={{ marginTop: 30 }}>
              ...
              <BarChart
                data={{
                  labels: topFiveStrands.map((item) => item.strand),
                  datasets: [{ data: topFiveStrands.map((item) => item.score) }],
                }}
